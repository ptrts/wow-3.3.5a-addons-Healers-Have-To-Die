...
for class, classColor in pairs(CLASS_COLORS) do
	classcolors[class] = {r = classColor.r, g = classColor.g, b = classColor.b}
end
...
function BattlegroundTargets:UpdateLayout()
	...
	for i = 1, currentSize do
		if ENEMY_Data[i] then
			...
			local currentEnemyData = ENEMY_Data[i]
			local currentClassToken = currentEnemyData.classToken
			local currentClassColor = classcolors[currentClassToken]
			--[[print("class")
			print(currentClassToken)
			print("classColor")
			print(currentClassColor)
			print("table")
			printTable(classcolors)]]
			local currentR = currentClassColor.r
			local currentG = currentClassColor.g
			local currentB = currentClassColor.b

			local targetButtons = GVAR.TargetButton
			local currentTargetButton = targetButtons[i]
			local currentClassColorBackground = currentTargetButton.ClassColorBackground

			-- Ставим цвет класса
			currentClassColorBackground:SetTexture(
					currentR,
					currentG,
					currentB,
					1
			...
			-- Иконка класса
			if BattlegroundTargets_Options.ButtonClassIcon[currentSize] then
				GVAR.TargetButton[i].ClassTexture:SetTexCoord(
						classes[ ENEMY_Data[i].classToken ].icon[1],
						classes[ ENEMY_Data[i].classToken ].icon[2],
						classes[ ENEMY_Data[i].classToken ].icon[3],
						classes[ ENEMY_Data[i].classToken ].icon[4]
				)
			end

			-- Иконка роли

			local currentSpec = currentEnemyData.talentSpec
			local currentRole = roles[currentSpec]

			GVAR.TargetButton[i].SpecTexture:SetTexCoord(
					currentRole[1],
					currentRole[2],
					currentRole[3],
					currentRole[4]
...
local function BattlefieldUpdateTargets(forceUpdate)
	...
	for index = 1, numScores do
		...
		classToken = classToken:gsub("%s+", ""):upper()
		...
		if name then
			...
			if faction == oppositeFaction then
				...
				--[[print("talentSpec")
				print(talentSpec)
				print(T[classToken][1])
				print(T[classToken][2])
				print(T[classToken][3])]]
...
function BattlegroundTargets:BattlefieldCheck()
	...
	if instanceType == "pvp" then
		...
		local isRatedBG = --[[IsRatedBattleground()]]false
...
function serializeTable(val, name, skipNewLines, depth)
	...
	elseif type(val) == "boolean" then
		...
		tmp = tmp .. "\"[not serializable datatype:" .. type(val) .. "]\""
	end
